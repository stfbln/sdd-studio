...
**References are picked, not typed.** Fields pointing to a single entity (owner, system, domain, repository, site, *produced by*…) list every entity they may point to when opened — with the kind and the reference next to the name — whether or not they are already filled, and typing narrows the list on the name, the reference or the kind. A reference can still be typed by hand, and the field says where it leads, or why it leads nowhere. **Dependencies** is a checklist of the components and resources of the catalog, next to the Networks, Data assets and Artifacts ones: tick what the entity depends on, say what it does with each ticked one, and let **New component** or **New resource** create one already ticked. A `dependsOn` entry pointing to no entity of the workspace stays listed, with why, so unticking it removes it.
