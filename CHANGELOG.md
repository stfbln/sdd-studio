...
- Markdown specs: a spec can **extend another spec**, so shared rules are written once and refined by the specs of each case (a data storage policy extended by an ephemeral storage and a persistent storage policy). An `Extends: [Title](path)` line under the title, written by a picker listing the specs of the workspace folder; the requirements of the whole chain are shown read-only in an **Inherited requirements** section, grouped by the spec that states them, with the ones the child overrides with another key word struck through. A requirement that repeats an inherited one, a missing or circular chain and a path outside the workspace folder are reported. New samples: `data-storage.spec.md`, `ephemeral-storage.spec.md` and `persistent-storage.spec.md`.
