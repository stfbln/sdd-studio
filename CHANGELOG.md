...
- Markdown specs: a spec can **extend one or several other specs**, so shared rules are written once and refined by the specs of each case (a data storage policy extended by an ephemeral storage policy, and by a persistent storage policy that also extends an audit logging policy). An `Extends: [Title](path), [Other](other-path)` line under the title, written by a field listing the specs extended and a picker of the other specs of the workspace folder; the requirements of every spec inherited from, directly or through another, are shown read-only in an **Inherited requirements** section, grouped by the spec that states them, with the ones the child overrides with another key word struck through. Where two specs extended disagree, the one listed first applies and the form says so. A requirement that repeats an inherited one, a spec extended that cannot be read or extends this one back, and a path outside the workspace folder are reported. New samples: `data-storage.spec.md`, `audit-logging.spec.md`, `ephemeral-storage.spec.md` and `persistent-storage.spec.md`.
